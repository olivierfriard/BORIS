        ...
        if plot_type == "spectrogram":
            if hasattr(self, "spectro"):
                ...
                if (dialog.MessageDialog(
                    ...
                    tmp_dir = (self.ffmpeg_cache_dir if self.ffmpeg_cache_dir and os.path.isdir(self.ffmpeg_cache_dir)
                ...
                if player == 0:
                    try:
                        self.video_slider.setValue(
                            round(self.dw_player[0].mediaplayer.get_time() /
        ...
        try:
            ...
            if preferencesWindow.exec_():
                ...
                if self.frame_resize != mem_frame_resize:
                    ...
                    if self.config_param.get(SAVE_FRAMES, DEFAULT_FRAME_MODE) == DISK:
                        # check temp dir for images from ffmpeg
                        self.imageDirectory = (self.ffmpeg_cache_dir if self.ffmpeg_cache_dir and
                                               os.path.isdir(self.ffmpeg_cache_dir) else tempfile.gettempdir())
        ...
        if hasattr(self, "measurement_w") and self.measurement_w is not None and self.measurement_w.isVisible():
            ...
            if self.measurement_w.rbDistance.isChecked():
                ...
                if event.button() == Qt.RightButton and self.memx != -1 and self.memy != -1:
                    ...
                    try:
                        distance = (distance / float(self.measurement_w.lePx.text()) *
                                    float(self.measurement_w.leRef.text()))
                    except Exception:
                        QMessageBox.critical(
                            self,
                            programName,
            ...
            if self.measurement_w.rbArea.isChecked():
                ...
                if event.button() == Qt.RightButton and len(self.memPoints) >= 2:
                    ...
                    try:
                        area = (area / (float(self.measurement_w.lePx.text())**2) *
    ...
    def readConfigFile(self):
        ...
        if iniFilePath.is_file():
            ...
            if self.checkForNewVersion:
                if settings.value("last_check_for_new_version") and (int(time.mktime(time.localtime())) - int(
        ...
        if flagGroup or len(selectedObservations) == 1:
            ...
            for element in out:
                ...
                if element["behavior"] in parameters[EXCLUDED_BEHAVIORS]:
                    excl_behaviors_total_time[element["subject"]] += (element["duration"] if
                                                                      not isinstance(element["duration"], str) else 0)
    ...
    def export_state_events_as_textgrid(self):
        ...
        try:
            ...
            for obsId in selectedObservations:
                ...
                if (mem_command != OVERWRITE_ALL and
                        pathlib.Path(f"{pathlib.Path(exportDir) / safeFileName(obsId)}.textGrid").is_file()):
                    if mem_command == "Skip all":
                        continue
                    mem_command = dialog.MessageDialog(
                        programName,
            ...
            if len(set(all_fps)) != 1:
                ...
                QMessageBox.warning(
                    ...
                    ("The frame-by-frame mode will not be available"
                     " because the video files have different frame rates ({}).").format(", ".join(map(str, all_fps))),
                    QMessageBox.Ok | QMessageBox.Default,
                    QMessageBox.NoButton,
                )
                self.actionFrame_by_frame.setChecked(False)
        ...
        if "from map" not in event:  # modifiers only for behaviors without coding map
            ...
            if event["modifiers"]:
                ...
                if [x for x in event["modifiers"] if event["modifiers"][x]["type"] != EXTERNAL_DATA_MODIFIER]:
                    ...
                    modifiers_selector = select_modifiers.ModifiersList(event["code"], eval(str(event["modifiers"])),
                                                                        currentModifiers)
