...
if versiontuple(platform.python_version()) < versiontuple("3.6"):
    logging.critical("BORIS requires Python 3.6+! You are using Python v. {}".format(platform.python_version()))
    sys.exit()
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def menu_options(self):
        ...
        self.setWindowTitle(
            "{}{}{}".format(
                self.observationId + " - " * (self.observationId != ""),
                pn + (" - " * (pn != "")),
                programName,
            )
        )
    ...
    def export_observations_list_clicked(self):
        ...
        file_name, filter_ = QFileDialog().getSaveFileName(
            self,
            "Export list of selected observations",
            "",
        ...
        for obs_id1 in selected_observations:
            for obs_id2 in selected_observations:
                ...
                if set([obs_id1, obs_id2]) not in mem_done:
                    ...
                    irr_results[
                        selected_observations.index(obs_id1),
                        selected_observations.index(obs_id2),
                    ] = K
                    irr_results[
                        selected_observations.index(obs_id2),
                        selected_observations.index(obs_id1),
    ...
    def events_snapshots(self):
        ...
        while True:
            text, ok = QInputDialog.getDouble(
                self,
                "Time interval around the events",
                "Time (in seconds):",
                0.0,
                0.0,
                86400,
    ...
    def show_waveform(self):
        ...
        if self.playerType == LIVE:
            QMessageBox.warning(
                self,
                programName,
                "The waveform visualization is not available for live observations",
            )
            return
        ...
        if self.playerType == VIEWER:
            QMessageBox.warning(
                self,
                programName,
                "The waveform visualization is not available in <b>VIEW</b> mode",
            )
            return
    ...
    def seek_mediaplayer(self, new_time: int, player=0):
        ...
        flag_paused = self.dw_player[player].mediaListPlayer.get_state() in [
            self.vlc_paused,
            self.vlc_ended,
        ]
        ...
        if self.playerType == VLC:
            ...
            if self.playMode == FFMPEG:
                ...
                if self.dw_player[0].media_list.index_of_item(self.dw_player[0].mediaplayer.get_media()) > 0:
                    ...
                    while True:
                        if self.dw_player[0].mediaListPlayer.get_state() in [
                            self.vlc_playing,
                            self.vlc_ended,
                        ]:
                            break
    ...
    def extract_frames_mem(
        ...
        try:
            ...
            while True:
                ...
                self.frames_cache[current_media_path][frame_idx] = (
                    start,
                    self.frames_buffer.size() - start,
                )
                frame_idx += 1
        ...
        for i, player in enumerate(self.dw_player):
            ...
            if self.config_param.get(SAVE_FRAMES, DEFAULT_FRAME_MODE) == MEMORY:
                ...
                if (
                    ...
                    self.statusbar.showMessage(
                        f"Extracting frames in memory {int(self.frames_buffer.size()/1024/1024)}",
                        0,
                    )
                    QApplication.processEvents()
                    """
    ...
    def initialize_new_observation_vlc(self):
        ...
        for i in range(N_PLAYER):
            ...
            if i < 4:
                self.addDockWidget(
                    Qt.TopDockWidgetArea if i < 4 else Qt.BottomDockWidgetArea,
                    self.dw_player[-1],
                )
                ...
                try:
                    ...
                    if "error" not in r:
                        if MEDIA_INFO not in self.pj[OBSERVATIONS][self.observationId]:
                            self.pj[OBSERVATIONS][self.observationId][MEDIA_INFO] = {
                                LENGTH: {},
                                FPS: {},
                            }
            ...
            if sys.platform != "darwin":
                while True:
                    if self.dw_player[i].mediaListPlayer.get_state() in [
                        self.vlc_playing,
                        self.vlc_ended,
                    ]:
                        break
    ...
    def new_observation(self, mode=NEW, obsId=""):
        ...
        if mode == EDIT:
            ...
            if self.pj[OBSERVATIONS][obsId].get(OBSERVATION_TIME_INTERVAL, [0, 0]) != [
                0,
                0,
            ]:
    ...
    def export_aggregated_events(self):
        ...
        if len(selectedObservations) > 1:
            flag_group = (
                dialog.MessageDialog(
                    programName,
                    "Group events from selected observations in one file?",
                    [YES, NO],
                )
                == YES
            )
        ...
        for obsId in selectedObservations:
            ...
            if not flag_group and outputFormat not in ["sds", "tbs"]:
                ...
                data = tablib.Dataset(
                    *sorted(list(data), key=lambda x: float(x[start_idx])),
                    headers=header,
                )
                data.title = obsId
    ...
    def export_state_events_as_textgrid(self):
        ...
        try:
            ...
            for obsId in selectedObservations:
                ...
                totalMediaDuration = round(
                    project_functions.observation_total_length(self.pj[OBSERVATIONS][obsId]),
                    3,
                )
        ...
        if self.observationId and self.playerType == VLC:
            ...
            for i, player in enumerate(self.dw_player):
                ...
                for filePath in self.pj[OBSERVATIONS][self.observationId][FILE][str(i + 1)]:
                    ...
                    r = utilities.accurate_media_analysis(self.ffmpeg_bin, media_full_path)
                    if "error" in r:
                        ffmpeg_output += f"File path: {media_full_path}<br><br>error{r['error']}<br><br>"
                    else:
                        ffmpeg_output += (
                            f"File path: {media_full_path}<br>"
                            f"Duration: {self.convertTime(r['duration'])}<br>"
            ...
            media_full_path = fn[0] if type(fn) is tuple else fn
            ...
            if media_full_path:
                ...
                r = utilities.accurate_media_analysis(self.ffmpeg_bin, media_full_path)
                if "error" in r:
                    self.results.ptText.appendHtml(f"File path: {media_full_path}<br><br>error{r['error']}<br><br>")
                else:
                    ...
                    for n_player, player in enumerate(self.dw_player):
                        ...
                        for idx, media in enumerate(self.pj[OBSERVATIONS][self.observationId][FILE][str(n_player + 1)]):
                            if self.FFmpegGlobalFrame < sum(player.media_durations[0 : idx + 1]):
                                ...
                                self.statusbar.showMessage(
                                    f"Snapshot player #1 saved in {snapshot_file_path}",
                                    0,
                                )
                                break
    ...
    def writeEvent(self, event: dict, memTime: Decimal) -> None:
        ...
        if position_in_events == len(self.pj[OBSERVATIONS][self.observationId][EVENTS]) - 1:
            ...
            self.twEvents.scrollToItem(
                self.twEvents.item(position_in_events, 0),
            ...
            if ok and text != "":
                ...
                if to_sec < from_sec:
                    QMessageBox.critical(
                        self,
                        programName,
                        "The initial time is greater than the final time",
                    )
                    return
        ...
        * frequencies_after_behaviors
        ...
        for subject in plot_parameters[SELECTED_SUBJECTS]:
            ...
            observed_matrix = transitions.observed_transitions_matrix(
                sequences,
                sorted(list(set(observed_behaviors + plot_parameters[SELECTED_BEHAVIORS]))),
                mode=mode,
            )
        ...
        try:
            for fileName in fileNames:
                with open(fileName, "r") as infile:
                    ...
                    with open(
                        tempfile.gettempdir() + os.sep + os.path.basename(fileName) + ".tmp.gv",
                        "w",
                    ) as f:
                        f.write(gv)
    ...
    def import_observations(self):
        ...
        if fileName:
            ...
            dbc = dialog.ChooseObservationsToImport(
                "Choose the observations to import:",
