...
from boris.analysis_plugins import irr_cohen_kappa_with_modifiers_new as plugin
...
def test_run_returns_perfect_agreement_for_identical_segments_with_modifiers(suppress_ui):
    df = _df(
        [
            ("obs1", 0.0, 1.0, "S", "A", "m1", ""),
            ("obs1", 1.0, 2.0, "S", "B", "", "n1"),
...
def test_modifier_value_changes_category_label(monkeypatch):
    monkeypatch.setattr(plugin.QInputDialog, "getInt", lambda *args, **kwargs: (3, True))
    ...
    expected = sklearn_cohen_kappa_score(["S|A|m1"], ["S|A|m2"])
    assert result.loc["obs1", "obs2"] == f"{expected:.3f}"


def test_instantaneous_events_are_added_as_point_observations_with_modifiers(monkeypatch):
    monkeypatch.setattr(plugin.QInputDialog, "getInt", lambda *args, **kwargs: (3, True))
    monkeypatch.setattr(plugin.QMessageBox, "warning", lambda *args, **kwargs: None)
    ...
    df = _df(
        [
            ("obs1", 0.0, 10.0, "S", "A", "seg"),
            ("obs1", 5.0, 5.0, "S", "A", "evt"),
            ("obs2", 0.0, 10.0, "S", "A", "seg"),
        ],
        modifier_columns=[MOD_A],
    )
    ...
    assert (["S|A|seg", "S|A|evt+S|A|seg"], ["S|A|seg", "S|A|seg"]) in captured_calls
