...
import irr_weighted_cohen_kappa_new as plugin
...
def test_weighted_run_returns_tuple_and_perfect_agreement(suppress_ui):
    df = _df(
        [
            ("obs1", 0.0, 1.0, "S", "A"),
            ("obs1", 1.0, 2.0, "S", "B"),
...
def test_event_weight_changes_disagreement_impact(monkeypatch):
    monkeypatch.setattr(plugin.QInputDialog, "getInt", lambda *args, **kwargs: (3, True))
...
def test_event_weight_falls_back_to_default_when_dialog_cancelled(monkeypatch):
    monkeypatch.setattr(plugin.QInputDialog, "getInt", lambda *args, **kwargs: (3, True))
    monkeypatch.setattr(plugin.QInputDialog, "getDouble", lambda *args, **kwargs: (9.5, False))
    monkeypatch.setattr(plugin.QMessageBox, "warning", lambda *args, **kwargs: None)
