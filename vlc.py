...
if sys.version_info[0] > 2:
    ...
    def str_to_bytes(s):
        ...
        if isinstance(s, str):
            ...
            Returned value was not UTF-8 compliant and VLC was unable to play file
            '''
    ...
    def str_to_bytes(s):
        ...
        if isinstance(s, str):
            ...
            return s.encode(fileSystemEncoding)
        else:
            return s
...
def find_lib():
    ...
    if sys.platform.startswith('linux'):
        ...
        print('Check for local VLC libraries in ' + os.path.abspath( os.path.join(sys.path[0] , os.pardir )) + "\VLC")
        if os.path.isdir( os.path.abspath( os.path.join(sys.path[0] , os.pardir )) + "\VLC"  ):
            mem_dir = os.getcwd()
            try:
                os.chdir( os.path.abspath( os.path.join(sys.path[0] , os.pardir )) + "\VLC" )
            except:
                print('VLC directory not accessible')
            try:
                dll = ctypes.CDLL('libvlc.dll')
                os.chdir(mem_dir)
                plugin_path = os.path.abspath( os.path.join(sys.path[0] , os.pardir )) + "\VLC"
        ...
        if dll is None:
            print('Check for local VLC libraries in ' + sys.path[0] + '\VLC')
            if os.path.isdir( sys.path[0] + '\VLC'):
                mem_dir = os.getcwd()
                os.chdir(sys.path[0] + '\VLC')
                try:
                    dll = ctypes.CDLL('libvlc.dll')
                    os.chdir(mem_dir)
                    plugin_path = sys.path[0] + '\VLC'
                    print("Local VLC libraries found in "+plugin_path)
                except OSError:
                    print('Local VLC libraries not found')
                    os.chdir(mem_dir)
        ...
        if dll is None:
            ...
            if p is None:
                ...
                try:  # some registry settings
                    ...
                    print('no pywin32')
                    pass
                print("plugin path 2", plugin_path)
                ...
                if plugin_path is None:
                    ...
                    for p in ('Program Files\\VideoLan\\', 'VideoLan\\',
                        ...
                        if os.path.exists(p):
                            ...
                            break
                print("plugin path 3", plugin_path)
...
class Instance(_Ctype):
    ...
    def __new__(cls, *args):
        ...
        ### added 2013-09-20 for linux deployement
        #args = ['vlc', '--plugin-path=/usr/lib/vlc/plugins' ]
